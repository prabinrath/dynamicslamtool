...
pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr MovingObjectDetection::tree = boost::shared_ptr<pcl::KdTreeFLANN<pcl::PointXYZI>>(new pcl::KdTreeFLANN<pcl::PointXYZI>);
...
void MovingObjectDetection::groundPlaneRemoval(float x,float y,float z)
{
...
void MovingObjectDetection::computeClusters(float distance_threshold, string f_id)
{
	clusters.clear();
...
void MovingObjectDetectionMethods::calculateCorrespondenceCentroid(pcl::PointCloud<pcl::PointXYZ>::Ptr fp, pcl::PointCloud<pcl::PointXYZ>::Ptr fc, pcl::CorrespondencesPtr mp,double delta)
{
